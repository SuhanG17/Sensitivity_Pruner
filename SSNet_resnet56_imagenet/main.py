...
parser.add_argument('--train_path', default='datasets/prune/train_loader', type=str, help='the path of dataset')
parser.add_argument('--val_path', default='datasets/prune/val_loader', type=str, help='the path of dataset')
...
parser.add_argument('--compression_rate', default=0.2, type=float, help='the percentage of 1 in compressed model') #0.2
...
def train(train_loader, ft_model, org_model, criterion1, criterion2, optimizer, epoch, regulization):
    ...
    singleshot_counter = []
    learned_counter = []
    for channels in channel_num[args.group_id]:
        singleshot_counter.append(np.zeros(channels))
        learned_counter.append(np.zeros(channels)) 
    # print(f'singleshot counter {len(singleshot_counter)}, {singleshot_counter}')
    # print(f'learned_counter {len(learned_counter)}, {learned_counter}')   
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        ssty, target_channel = Comp_ssty.Setting_Sensity(org_model, input_var, target_var, args.compression_rate, args.group_id, args.layersort, gpu_num)
        ...
        if len(channel_index_list) == args.channel_index_range:
            ...
            for j in range(k):
                ...
                ## save binarized channel_index every channel_index_range epochs
                ...
                if channel_index.sum() == 0:
                    idx_to_save = np.argsort(tmp)[-1] # return the index of the largest to prevent zero channel prunning
                    channel_index[idx_to_save] = 1
                ...
                if cur_two_side_rate < 0.9 and beta_index >= int(beta_list.shape[1] / args.num_epochs):
                    beta_list[j, :] = beta_list[j, :] + 1 #default +1
            ...
            # SG channel_index vs mask_vec
            # print(f'channel index binary {len(channel_index_binary)}, {channel_index_binary[0].shape} {channel_index_binary[0]}')
            # print(f'target_channel {len(target_channel)}, {target_channel[0].shape}, {target_channel[0]}')

            # agreement_counter = 0
            # for k in range(len(channel_index_binary)):
            #     singleshot = target_channel[k].data.cpu().numpy()
            #     learned = channel_index_binary[k]
            #     agreement = (singleshot == learned).sum() / singleshot.shape[0]
            #     print(f'pruned channel {k} has agreement {agreement}')
            #     agreement_counter += agreement

            #     singleshot_counter[k] += singleshot
            #     learned_counter[k] += learned

            # if i == 20000-1:
            #     path = 'results/models/counter'
            #     torch.save(singleshot_counter, path+ '/singleshot_counter_group_' + str(args.group_id) + '_epoch_' + str(epoch) + '.pth' )
            #     torch.save(learned_counter, path+ '/learned_counter_group_' + str(args.group_id) + '_epoch_' + str(epoch) + '.pth')
            # # print(f'at sample {i} singleshot counter {len(singleshot_counter)}, {singleshot_counter}')
            # # print(f'at sample {i} learned_counter {len(learned_counter)}, {learned_counter}')
            # print(f'Epoch {epoch} Sample {i}: average agreement for this group is {round(agreement_counter/len(channel_index_binary), 3)}')
