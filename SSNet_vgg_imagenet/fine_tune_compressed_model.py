...
parser.add_argument('--train_path', default='datasets_64/finetune/train_loader', type=str, help='the path of dataset')
parser.add_argument('--val_path', default='datasets_64/finetune/val_loader', type=str, help='the path of dataset')
...
def main():
    ...
    args = parser.parse_args()
    model = SsNet_test('results/models/model.pth')
    
    if (args.resume is not None):
        model = SsNet_test(args.resume)
        
    model = torch.nn.DataParallel(model.to(device), device_ids=range(torch.cuda.device_count()))
    ...
    # optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), 0.001,
    #                         momentum=0.9,
    #                         weight_decay=12-4)
