...
class Use_Sensity(nn.Module):
    ...
    def forward(self, x, ssty, beta, channel_index=None):
        """ if torch.nn.DataParallel, ssty and beta should be on the device, where x is on 
        beta is an one element tensor, with shape [], does not require grad
        """
        if self.training:
            ssty = torch.sum(ssty,dim=0).unsqueeze(dim=0).to(x.device)
            new_mask = self.conv(ssty)
            new_mask = torch.squeeze(new_mask)
            
            new_mask = new_mask * beta.to(x.device)
            new_mask = self.sigmoid(new_mask).clone()

        else:
            new_mask = None
            new_mask = torch.FloatTensor(channel_index.mean(0).squeeze().cpu()).cuda()
