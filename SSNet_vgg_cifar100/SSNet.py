...
class Vgg16(torch.nn.Module):
    def __init__(self, model_path):
        ...
        self.classifier.add_module('fc6', nn.Linear(512, 4096))
        ...
        self.classifier.add_module('fc8', nn.Linear(4096, 100))
    ...
    def __init__(self, layer_id=0):
        ...
        for layer in range(13):
            if layer <= layer_id:
                ...
                if layer in pooling_layer_id:
                    self.feature_2.add_module(pool_names[layer],nn.MaxPool2d(kernel_size=2,stride=2))
        self.classifier.add_module(fc_names[13], nn.Linear(channel_len[13], channel_len[14]))
        self.classifier.add_module(relu_names[13], nn.ReLU(inplace=True))
        self.classifier.add_module(drop_names[13], nn.Dropout())
        self.classifier.add_module(fc_names[14], nn.Linear(channel_len[14], channel_len[15]))
        self.classifier.add_module(relu_names[14], nn.ReLU(inplace=True))
        self.classifier.add_module(drop_names[14], nn.Dropout())
        self.classifier.add_module(fc_names[15], nn.Linear(channel_len[15], channel_len[16]))
    ...
    def __init__(self, layer_id=13):
        ...
        for layer in range(13):
            ...
            if layer in pooling_layer_id:
                self.feature_1.add_module(pool_names[layer],nn.MaxPool2d(kernel_size=2,stride=2))
        layer = 13
        self.classifier_1.add_module(fc_names[layer], nn.Linear(channel_len[layer], channel_len[layer+1]))
        self.classifier_1.add_module(relu_names[layer], nn.ReLU(inplace=True))
        self.classifier_1.add_module(drop_names[layer], nn.Dropout())
        layer = 14
        if layer_id == 13:
            self.classifier_2.add_module(fc_names[layer], nn.Linear(channel_len[layer], channel_len[layer+1]))
            self.classifier_2.add_module(relu_names[layer], nn.ReLU(inplace=True))
            self.classifier_2.add_module(drop_names[layer], nn.Dropout())
        elif layer_id == 14:
            self.classifier_1.add_module(fc_names[layer], nn.Linear(channel_len[layer], channel_len[layer+1]))
            self.classifier_1.add_module(relu_names[layer], nn.ReLU(inplace=True))
            self.classifier_1.add_module(drop_names[layer], nn.Dropout())
