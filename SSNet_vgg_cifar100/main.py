...
parser.add_argument('--data_base', default='/nas/laibl/data', type=str, help='the path of dataset')
parser.add_argument('--ft_model_path', default='results/models/model_vgg16.pth',
                    type=str, help='the path of fine tuned model')
parser.add_argument('--gpu_id', default='4,0,6', type=str,
...
channel_len = [3, 64, 64, 128, 128, 256, 256, 256, 512, 512, 512, 512, 512, 512, 4096, 4096, 100]

def main():
    print('Data Preparing...')
    global args, best_prec1, beta_list, device
    normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5],
                                    std=[0.5, 0.5, 0.5])
    train_loader = torch.utils.data.DataLoader(
        datasets.CIFAR100(root='/nas/laibl/data/cifar100', train=True, transform=transforms.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.CIFAR100(root='/nas/laibl/data/cifar100', train=False, transform=transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ]),download=True),
        batch_size=args.batch_size, shuffle=True,
    ...
    print('Model Loading...')
    if args.layer_id == 0:
        model = SSNet.Vgg16(args.ft_model_path)
    ...
    beta_list = np.linspace(0.01,10, int(args.num_epochs * len(train_loader) / args.beta_range))
    regulization = 0.1
    ...
    for epoch in range(args.start_epoch,args.num_epochs):
        ...
        prec1 = validate(val_loader, model, criterion1, channel_index)
...
def train(train_loader, model, old_model, criterion1, criterion2, optimizer, epoch, regulization):
    global beta_list, beta_idx, device
    ...
    for i,(inputs, targets) in enumerate(train_loader):
        ...
        tmp = mask_vec.data.cpu().numpy().reshape(-1,channel_len[args.layer_id+1]).mean(0)
...
def validate(val_loader, model, criterion, channel_index):
    ...
    for i, (inputs, targets) in enumerate(val_loader):
        ...
        channel_index_ = torch.repeat_interleave(torch.from_numpy(channel_index).unsqueeze(dim=0), repeats=input_var.shape[0], dim=0 )
        channel_index_ = torch.autograd.Variable(channel_index_).to(device)

        # 计算输出
        # ssty, target_channel = Comp_ssty.Setting_Sensity(old_model, input_var, target_var, args.compression_rate, args.layer_id)
        output, _ = model(input_var, None, 1.0, channel_index_)
