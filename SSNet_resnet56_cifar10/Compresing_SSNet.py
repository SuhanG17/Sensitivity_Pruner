...
class ResNet(nn.Module):
    ...
    def __init__(self, group_id, block, layers, num_classes=10):
        ...
        for i in range(min(int(len(channel_index)/2),resnet_channel_number[group_id]//2)):
...
class ResNet_test(nn.Module):
    ...
    def __init__(self, model_path, block, layers, num_classes=10, flag=True):
        old_model =  torch.load(model_path)
        # old_weight = old_model.state_dict()
        old_weight = old_model
        # else:
            # old_weight = old_model
        channel_number_list = analyse_number(old_weight)
        self.flag = flag
...
def Resnet_test(model_path, flag=True):
    model = ResNet_test(model_path, Bottleneck, [3, 4, 6, 3], flag=flag)
    return model
