...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=10):
        ...
        old_model = torch.load('results/models/model_9389.pth')
        ...
        # org_keys = list(old_weight.keys())
        # for i in range(len(my_keys)):
        #     print(my_keys[i],org_keys[i])
        for k, v in old_weight.items():
            if 'shortcut' in k:
                name = k[7:]
                name = name.split('shortcut')
                name = 'downsample'.join(name)
            elif 'linear' not in k:
                name = k[7:]
            else:
                name = k.split('.')
                name = 'fc.' + name[-1]
