...
parser.add_argument('--ft_model_path', default='/nas/laibl/AutoPruner-master/ResNet50/50/checkpoint/pre_model.pth',
                    type=str, help='the path of fine tuned model')
parser.add_argument('--gpu_id', default='1,0', type=str,
...
parser.add_argument('--compression_rate', default=0.6, type=float, help='the percentage of 1 in compressed model')
...
def main():
    ...
    print('Model setup')
    if args.group_id == 0:
        model_ft = SSNet_ResNet.ResNet50()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if len(channel_index_list) == args.channel_index_range:
            ...
            for j in range(k):
                ...
                if cur_two_side_rate < 0.9 and beta_index >= 2 * int(beta_list.shape[1] / args.num_epochs):
...
def validate(val_loader, model, criterion, channel_index):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        output, _ = model(input_var, None, channel_index)
