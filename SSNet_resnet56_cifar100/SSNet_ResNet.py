...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100):
        ...
        old_model = torch.load('results/models/model_resnet50.pth')
        old_weight = old_model.state_dict()
        channel_number_list = analyse_number_org(old_weight)
        self.inplanes = 64
...
class ResNet_SSNet(nn.Module):
    def __init__(self, group_id, block, layers, num_classes=100):
        ...
        # self.all_channel_num = (64*3 + 128*4 + 256*6 + 512*3)*2
        # cur_channel_num = 0
        # for i in range(4):
        #     for j in range(len(channel_number_list[i])):
        #         cur_channel_num += sum(channel_number_list[i][j][0:4:2])
        # self.ratio = cur_channel_num/self.all_channel_num
        self.inplanes = 64
        self.g_id = group_id
...
def analyse_number_org(weight):
    ...
    for k, v in weight.items():
        name = k.split('.')
        if '_x' in k and 'residual' in k and name[4] in ['0', '3', '6']:
            current_name = k.split('conv')[1].split('residual')[0]
...
def analyse_number(weight):
    ...
    # channel_num = []
    for k, v in weight.items():
        if 'layer' in k and'conv' in k and 'Sensity' not in k:
